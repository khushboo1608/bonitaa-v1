...
<body>
  ...
  <?php include('content/reschedule_order_details.php'); ?>
  ...
  $(document).ready(function(){
    ...
    $(".submit_checkout").on("click",function(e){
        ...
        if (!date) {
            ...
            alert('Undefined booking id');
        }else if (!timezone) {
            alert('Select Your Slot or time');
        }else{
           
            $.ajax({
            url: "public/update_reschedule_order.php",
            method: "GET",
            data: { date: date, timezone:timezone, book_id:book_id },
            success: function (data) {
            //   alert(data);
              if (data == 1) {
                window.location = "single_appointment.php?book="+book_id;
              }else{
