...
$pageurl      = "pages$extn";
$tblname      = "pages";
$pagename     = "Dynamic Pages";
$id           = @$_GET['id'];
$pic_required = '';
$sessionid    = date("ymdHis");
// Get POST Values
$title    = get_safe_value($con,@$_POST['title']);
$surl     = createUrl($title);
...
$newfilename = "page-".date("ymdHis");
//Determine the path to which we want to save this file
$ext=".".$ext;
$newname = '../uploads/pages/'.$newfilename.$ext;
...
$sqlview = mysqli_query($con,"SELECT * FROM `$tblname` WHERE page_name='$title'");
$check = mysqli_num_rows($sqlview);
if($check>0){
echo"<META HTTP-EQUIV='REFRESH' CONTENT='0; URL=$pageurl?dupl_msg=$pagename Already Exist...'>";  exit(0);
}else{
$sql = "INSERT INTO `$tblname`(`page_name`, `page_url`, `details`, `img`, `createdon`)
VALUES ('$title', '$surl', '$desc', '$pic', '$now')";
...
$img_required ="";
$sql="UPDATE `$tblname` SET  `page_name`='$title', `page_url`='$surl', `details`='$desc', `modifiedon`='$now' WHERE ID=$id";
if (!mysqli_query($con,$sql)){die('Error: ' . mysqli_error($con)); }
...
$sqlx="UPDATE `$tblname` SET `img`='$pic' WHERE ID='$id'";
...
// debug($row);
$get_title = $row['page_name'];
$get_desc  = $row['details'];
$get_pic   = $row['img'];
}
}else{
                  ...
                  <form role="form" method="POST" action="<?= $pageurl; ?>?mode=update&id=<?= @$_GET['id'] ?>" enctype="multipart/form-data">
                    ...
                    <form role="form" method="POST" action="<?= $pageurl; ?>?mode=addnew" enctype="multipart/form-data">
                      ...
                      <div class="box-body">
                        ...
                        <div class="form-group col-md-12">
                          <label for="page">Page Title <span class="text-red">*</span></label>
                          <input type="text" class="form-control" name="title" placeholder="Page title" value="<?= @$get_title; ?>" required autocomplete="off">
                        </div>
                        <div class="form-group col-md-12">
                          <label for="description"> Page Description <span class="text-red">*</span></label>
                          <textarea id="tinyeditor" name="description" rows="5" class="form-control" placeholder="Enter Course Description" required style="height: 400px;"><?= @$get_desc; ?></textarea>
                        ...
                        <?php if(@$_GET['mode']=="edit" && @$_GET['id']!="" && @$get_pic!=NULL) { ?>
                        <div class="form-group col-md-6">
                          <img src="<?= UPLOAD_PATH.'pages/'.$get_pic; ?>" width="100" height="100" alt="" style="border: 1px solid grey;padding: 2px; border-radius: 10px;">
                        </div>
                        <?php } ?>
                      </div>
                      <!-- /.box-body -->
                  ...
                  <table id="example1_samy" class="table table-bordered table-striped table-hover" style="text-align: center;">
                    <thead>
                      ...
                      <tr>
                        ...
                        <th>Page Title & DeDescription</th>
                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td style="text-align: left;">
                          <span class="bold font-14"><?php echo $row['page_name']; ?></span> <br>
                          <span class="text-red bold ps12">Url: </span><?php echo $row['page_url']; ?>
                        </td>
