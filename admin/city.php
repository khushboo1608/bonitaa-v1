...
$convenience_fee    = get_safe_value($con,@$_POST['convenience_fee']);
$safety_charge    = get_safe_value($con,@$_POST['safety_charge']);
...
$img_required ="";
  $sql="UPDATE `$tblname` SET  `convenience_fee`='$convenience_fee',`safety_charge`='$safety_charge' ,`state_id`='$state_id',`name`='$title' WHERE ID =$id";
  if (!mysqli_query($con,$sql)){die('Error: ' . mysqli_error($con)); }
...
//Select query when we click on pencil for edit existing entry
  ...
  if($count>0){
    while($row = mysqli_fetch_array($result)) {
      ...
      $get_title    = $row['name'];
      $get_convenience_fee    = $row['convenience_fee'];
      $get_safety_charge   = $row['safety_charge'];
          ...
          <div class="row">
            ...
            <?php } ?>
              <div class="box-body">
                ...
                <div class="form-group col-md-4">
                  <label for="name">Convenience fee <span class="text-red">*</span></label>
                  <input type="text" class="form-control" name="convenience_fee" value="<?= @$get_convenience_fee; ?>" autocomplete="off" placeholder="Convenience fees" required>
                </div>

                <div class="form-group col-md-4">
                  <label for="name">Safety charge <span class="text-red">*</span></label>
                  <input type="text" class="form-control" name="safety_charge" value="<?= @$get_safety_charge; ?>" autocomplete="off" placeholder="Safety charge" required>
                </div>
          ...
          <table id="example1_samy" class="table table-bordered table-striped table-hover">
            <thead>
              ...
              <tr>
                ...
                <th width="20">Name</th>
                <th width="20">Convenience fee</th>
                <th width="20">Safety charge</th>
            ...
            <tbody>
              ...
              <tr>
                ...
                <td ><?php echo $row['convenience_fee']; ?></td>
                <td ><?php echo $row['safety_charge']; ?></td>
                <td>
