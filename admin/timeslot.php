...
// Get POST Values
$timeslot    = get_safe_value($con,@$_POST['timeslot']);
$slot_max_book    = get_safe_value($con,@$_POST['slot_max_book']);

// Insert new entry Query
if(@$_GET['mode']=="addnew"){
    // INSERT INTO `timeslot`(`ID`, `date`, `timeslot`, `slot_max_book`, `createdon`) 
    $sql = "INSERT INTO `$tblname`(`date`, `timeslot`, `slot_max_book`) 
    VALUES ('$dd', '$timeslot','$slot_max_book')";
...
$img_required ="";
  $sql="UPDATE `$tblname` SET `timeslot`='$timeslot', slot_max_book='$slot_max_book'  WHERE ID =$id";
  if (!mysqli_query($con,$sql)){die('Error: ' . mysqli_error($con)); }
...
//Select query when we click on pencil for edit existing entry
  ...
  if($count>0){
    while($row = mysqli_fetch_array($result)) {
      ...
      $get_timeslot = $row['timeslot'];
      $get_slot_max_book = $row['slot_max_book'];
            ...
            <?php } ?>
              <div class="box-body">
                <div class="form-group col-md-4">
                  ...
                  <input type="text" class="form-control" name="timeslot" value="<?= @$get_timeslot; ?>" autocomplete="off" placeholder="Timeslot" required>
                </div>
              </div>

              <div class="box-body">
                <div class="form-group col-md-4">
                  <label for="name">Max slot book <span class="text-red">*</span></label>
                  <input type="text" class="form-control" name="slot_max_book" value="<?= @$get_slot_max_book; ?>" autocomplete="off" placeholder="order per maximum book" required>
                </div>
              </div>
          ...
          <table id="example1_samy" class="table table-bordered table-striped table-hover">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td valign="middle"> <?php echo $row['timeslot']; ?> </td>

                <td valign="middle"> <?php echo $row['slot_max_book']; ?> </td>
