...
if(@$_GET['mode']=="addnew"){
    $sql = "INSERT INTO `$tblname`(`status`, `state_id`, `city_id`, `name`)
    VALUES ('1','$state_id', '$city_id', '$title')";
...
$img_required ="";
  $sql="UPDATE `$tblname` SET `state_id`='$state_id',`city_id`='$city_id',`name`='$title' WHERE ID =$id";
  if (!mysqli_query($con,$sql)){die('Error: ' . mysqli_error($con)); }
...
$sql = "SELECT p.*,s.name as state_name,c.name as city_name FROM `$tblname` p
            ...
            <?php } ?>
              <div class="box-body">
                ...
                <div class="form-group col-md-4">
                  ...
                  <input type="text" class="form-control" name="title" value="<?= @$get_title; ?>" autocomplete="off" placeholder="Enter Pincode" required>
                </div>
            ...
            <tbody>
              ...
              <tr>
                ...
                <td valign="middle">
                  ...
                  <span class="text-pink bold font-15">City Name: </span><?php echo $row['city_name']; ?><br>
                  <span class="text-red bold font-15">Pincode: </span><?php echo $row['name']; ?><br>
...
<script>
  $(document).ready(function(){
    ...
    var get_scid = "";
    console.log("Pincode ID: "+ get_scid);

      var id=jQuery(this).val();
      console.log("State ID: "+ id);
      if(id=='-1'){
        jQuery('#city_id').html('<option value="-1">Select City </option>');
      }else{
        $("#divLoading").addClass('show');
        jQuery('#city_id').html('<option value="-1">Select City </option>');
        jQuery.ajax({
          type:'post',
          url:'get_data2.php?mode=city_id',
          data:'id='+id+'&get_scid='+get_scid,
          // data:'id='+id,
          success:function(result){
            $("#divLoading").removeClass('show');
            jQuery('#city_id').append(result);
          }
        });
      }
    });
