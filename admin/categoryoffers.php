<?php 
require('check.php');
$pageurl      = "categoryoffers$extn";
$tblname      = "cat_offer";
$pagename     = "Category Offers";
$id           = @$_GET['id']; 
$pic_required = '';
...
if((!empty($_FILES["pic"])) && ($_FILES['pic']['error'] == 0)){
  ...
  $newfilename = "catoffers-".date("ymdHis");
  //Determine the path to which we want to save this file
  $ext=".".$ext;
  $newname = '../uploads/cat_offers/'.$newfilename.$ext;
...
if(@$_GET['mode']=="addnew"){
 $sql = "INSERT INTO `$tblname`(`status`,`type`,`category_id`,`subcategory_id`, `subsubcategory_id`, `title`,`click`,`pic`,`height`,`width`,`createdon`,`sort`) 
    VALUES ('1','$type', '$category_id', '$sub_category_id', '$sub_sub_sub_category_id','$title','$click', '$pic','$height','$width', '$now', '$sort')";
...
$img_required ="";
    $sql="UPDATE `$tblname` SET `type`='$type', `title`='$title',`click`='$click',`category_id`='$category_id',`subcategory_id`='$sub_category_id',`subsubcategory_id`='$sub_sub_sub_category_id', `sort`='$sort', `modifiedon`='$now',`height`='$height',`width`='$width' WHERE ID =$id";
  if (!mysqli_query($con,$sql)){die('Error: ' . mysqli_error($con)); }
...
$sql = "SELECT *,c.ID,subcat.ID,subsubcat.ID,c.category as cname,co.pic as oimage,co.ID,co.status,subcat.subcategory as subcat_name,subsubcat.subsubcategory_name as subsubsubcat_name,co.sort as osort,co.modifiedon,co.height,co.width FROM `cat_offer` co 
...
WHERE co.hide = 0 ORDER BY co.ID DESC";
$result = mysqli_query($con,$sql);   
              ...
              <div class="box-body">
                ...
                <div class="form-group col-md-4">
                  ...
                  <select name="type" id="type" class="form-control">
                    ...
                    <option value="1" <?php if(@$get_type=="1"){?> selected="selected"<?php } ?>>APP</option>
                    <option value="2" <?php if(@$get_type=="2"){?> selected="selected"<?php } ?>>Website</option>
                  </select>
                </div>
                ...
                <div class="form-group col-md-4 category_id" >
                ...
                <div class="form-group col-md-3">
                  <img src="<?= UPLOAD_PATH.'cat_offers/'.$get_pic; ?>" width="200" height="100" alt="" style="border: 1px solid grey;padding: 2px; border-radius: 10px;">
                </div>
                <?php } ?>
          ...
          <table id="example1_samy" class="table table-bordered table-striped table-hover" style="text-align: center;">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td align="center"><?php echo $row['osort']; ?></td>
                <td align="left">
                  <span class="text-green bold font-15">Type: </span><span><?php if($row['type'] == 1){echo 'APP';}else if($row['type'] == 2){echo 'Website';} ?></span><br>
                ...
                <td align="center">
                  <a href="<?= UPLOAD_PATH.'cat_offers/'.$pic; ?>" target="_blank">
                    <img src="<?= UPLOAD_PATH.'cat_offers/'.$pic; ?>" width="200" style="max-height: 150px;">
                  </a>
                </td>
...
<script>
  $(document).ready(function(){
    ...
    console.log("SubCategory ID: "+ get_scid);
      ...
      if(id=='-1'){
        jQuery('#subcategoryid').html('<option value="0">Select SubCategory </option>');
      }else{
...
<script>
  $(document).ready(function(){
    ...
    console.log("SubSubCategory ID: "+ get_sscid);
      ...
      console.log("Category ID: "+ id);
      if(id=='-1'){
        jQuery('#subsubcategoryid').html('<option value="0">Select SubSubCategory </option>');
      }else{
        $("#divLoading").addClass('show');
        jQuery('#subsubcategoryid').html('<option value="0">Select SubSubCategory </option>');
        jQuery.ajax({
