<?php 
require('check.php');
$pageurl      = "product_staff_inventory$extn";
$tblname      = "product_staff_inventory";
$pagename     = "Product Staff Inventory";
$id           = @$_GET['id']; 
$pic_required = '';
...
$staff_stock_qty     = get_safe_value($con,@$_POST['staff_stock_qty']);
$staff_sell_qty     = get_safe_value($con,@$_POST['staff_sell_qty']);


// Insert new entry Query
if(@$_GET['mode']=="addnew"){
  $sql = "INSERT INTO `$tblname`(`staff_inventory_status`, `product_id`, `staff_id`, `staff_stock_qty`, `staff_sell_qty`, `staff_inventory_date`) VALUES ('1', '$product_id', '$staff_id', '$staff_stock_qty' , '$staff_sell_qty', '')";
...
$img_required ="";
  $sql="UPDATE `$tblname` SET `product_id`='$product_id', `staff_id`='$staff_id', `staff_stock_qty`='$staff_stock_qty', staff_sell_qty='$staff_sell_qty' WHERE staff_inventory_id =$id";
  if (!mysqli_query($con,$sql)){die('Error: ' . mysqli_error($con)); }
}
...
// Statue Update Query
  ...
  $sql="UPDATE `$tblname` SET staff_inventory_status ='$status' WHERE staff_inventory_id ='$id'";
...
// Delete Query
   ...
   $sql = "DELETE FROM `$tblname` WHERE staff_inventory_id='$id'";
...
//Select query when we click on pencil for edit existing entry
  ...
  $pic_required = '';
  $sql = "SELECT * FROM `$tblname` WHERE staff_inventory_id='$id'";
  ...
  if($count>0){
    while($row = mysqli_fetch_array($result)) {
      ...
      $get_staff_stock_qty     = $row['staff_stock_qty'];
      $get_staff_sell_qty     = $row['staff_sell_qty'];
      $get_staff_inventory_status    = $row['staff_inventory_status'];
      $get_staff_inventory_date      = $row['staff_inventory_date'];
    }
  }else{
...
$sql = "SELECT * FROM `product_staff_inventory`
    LEFT JOIN staff on staff.ID=product_staff_inventory.staff_id
    LEFT JOIN product on product.product_id=product_staff_inventory.staff_id
          ...
          <table id="example1_samy" class="table table-bordered table-striped table-hover" style="text-align: center;">
            <thead>
              ...
              <tr>
                ...
                <th width="120">Used Stock</th>
                <th width="120">Live Stock</th>
            ...
            <tbody>
              <?php $num=0; while($row=mysqli_fetch_array($result)){ $num=$num+1;
                $pic = ($row['product_image']!="") ? $row['product_image'] : "noimg.webp";
               ?>
              <tr>
                ...
                <?php if($row['product_image'] !=""){ ?>
                  <a href="<?= UPLOAD_PATH.'products/'.$pic; ?>" target="_blank">
                    <img src="<?= UPLOAD_PATH.'products/'.$pic; ?>" width="50" height="50" style="border-radius:10%;">
                  </a>
                <?php } ?>
                </td>

                <td style="text-align: left;"><?php echo $row['staff_stock_qty']; ?></td>
                <td style="text-align: left;"><?php echo $row['staff_sell_qty']; ?></td>
                <td style="text-align: left;">
                  <?php 
                    if($row['staff_stock_qty'] >= $row['staff_sell_qty'] ){
                      echo $remaining_stock = $row['staff_stock_qty'] - $row['staff_sell_qty'];
                    }else{
                      echo $remaining_stock = "0";
                    }
                  ?>
                </td>
                <td>
                  <?php  
                    if($row['staff_inventory_status']=='1'){
                      echo "<a href='$pageurl?mode=update_status&status=0&id=".$row['staff_inventory_id']."'><small class='label bg-green'>Active</small></a>";
                    }else{
                      echo "<a href='$pageurl?mode=update_status&status=1&id=".$row['staff_inventory_id']."'><small class='label bg-red'>Inactive</small></a>";
                    }
                  ?>
                </td>
                ...
                <td>
                  <!-- <a href="<?= $pageurl; ?>?mode=edit&id=<?php echo $row['staff_inventory_id'] ?>" title="Edit"><i class="fa fa-edit"></i>&nbsp;</a> -->
                  <a onClick="return verifCompare();" href="<?= $pageurl; ?>?id=<?php echo $row['staff_inventory_id'] ?>&mode=delete" class="text-red" title="Delete"><i class="fa fa-trash-o"></i>&nbsp;</a>
                </td>
              </tr>
            <?php } ?>
            </tbody>
