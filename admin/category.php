...
// Get POST Values
$title   = get_safe_value($con,@$_POST['category']);
$desc    = get_safe_value($con,@$_POST['description']);
$cat_url = createUrl($title);
...
if((!empty($_FILES["cover_img"])) && ($_FILES['cover_img']['error'] == 0)){
  ...
  $newfilename = "cat-".date("ymdHis");
  //Determine the path to which we want to save this file
  $ext=".".$ext;
  $newname = '../uploads/category/'.$newfilename.$ext;
...
if(@$_GET['mode']=="addnew"){
  ...
  $sql = mysqli_query($con,"SELECT * FROM `$tblname` WHERE category='$title'");
  $check = mysqli_num_rows($sql);
  if($check>0){
    echo"<META HTTP-EQUIV='REFRESH' CONTENT='0; URL=$pageurl?dupl_msg=$pagename Already Exist...'>";  exit(0);
  }else{
    $sql = "INSERT INTO `$tblname`(`category`, `categoryurl`, `description`, `pic`, `createdon`, `rd`, `date`, `time`, `sort`) 
    VALUES ('$title', '$cat_url', '$desc', '$pic', '$now', '$rd', '$dd', '$dt', '1')";
...
$img_required ="";
  $sql="UPDATE `$tblname` SET `category`='$title', `categoryurl`='$cat_url', `description`='$desc', `modifiedon`='$now' WHERE ID =$id";
  if (!mysqli_query($con,$sql)){die('Error: ' . mysqli_error($con)); }
            ...
            <?php } ?>
              <div class="box-body">
                <div class="form-group">
                  ...
                  <input type="text" class="form-control" name="category" value="<?= @$get_category; ?>" autocomplete="off" placeholder="Category Title">
                </div>
                <!--<div class="form-group">-->
                ...
                <div class="form-group col-md-3">
                  <img src="<?= UPLOAD_PATH.'category/'.$get_pic; ?>" width="100" height="100" alt="" style="border: 1px solid grey;padding: 2px; border-radius: 10px;">
                </div>
                <?php } ?>
              </div>
              <!-- /.box-body -->
          ...
          <table id="example1_samy" class="table table-bordered table-striped table-hover">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td align="center">
                  <img src="<?= UPLOAD_PATH.'category/'.$cpic; ?>" width="50" height="50" style="border-radius:10%;">
                </td>
