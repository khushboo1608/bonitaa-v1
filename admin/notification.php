...
// Get POST Values
$title   = get_safe_value($con,@$_POST['tittle']);
...
if((!empty($_FILES["cover_img"])) && ($_FILES['cover_img']['error'] == 0)){
  ...
  $newfilename = "noti-".date("ymdHis");
  //Determine the path to which we want to save this file
  $ext=".".$ext;
  $newname = '../uploads/notification/'.$newfilename.$ext;
...
if(@$_GET['mode']=="addnew"){
    ...
    $file_path = UPLOAD_PATH.'notification/'.$pic;
    // exit;
    $content = array(
        "en" => $msg                                                
    );
  ...
  // check for duplicate entry is there or not
    ...
    $sql = "INSERT INTO `$tblname`(`status`, `click`, `type`, `user_id`, `order_id`, `tittle`, `date`, `msg`, `image`)
    VALUES ('1','0','0','0','0','$title', '$now','$msg','$pic')";
            ...
            <?php } ?>
              <div class="box-body">
                <div class="form-group">
                  ...
                  <input type="text" class="form-control" name="tittle"  autocomplete="off" placeholder="Notification Title">
                </div>
                <div class="form-group">
                  <label for="desc">Message <i class="font-12">[Optional] </i></label>
                  <textarea name="msg" id="details" rows="5" class="form-control" placeholder="Enter Notification Message Here"></textarea>
                ...
                <div class="form-group col-md-6">
                  ...
                  <span class="btn btn-primary margin">
                    <input type="file" name="cover_img" id="cover_img" class="img" style="width:250px;" >
                  </span>
                </div>
          ...
          <table id="example1_samy" class="table table-bordered table-striped table-hover">
            ...
            <tbody>
              ...
              while($row=mysqli_fetch_array($result)){ $num=$num+1; 
                $cpic = ($row['image']!="") ? $row['image'] : "noimg.webp";
                ?>
              <tr>
                ...
                <td align="center">
                  <?php if($row['image']!=""){ ?>
                  <img src="<?= UPLOAD_PATH.'notification/'.$cpic; ?>" width="50" height="50" style="border-radius:10%;">
                  <?php } ?>
                </td>
                <td style="text-align: left;"><?php echo $row['tittle']; ?></td>
                <td>
                  <!--<span class="text-red bold ps12">Slug: </span><?php echo $row['categoryurl']; ?><br>-->
                  <?php if($row['msg']!=""){ ?>
                  <span class="text-blue bold ps12">Message: </span><span class="ps13"><?php echo $row['msg']; ?></span>
                  <?php } ?>
                </td>
                <td title="Modified on: <?php echo $row['date']; ?>"><?php echo formatDate($row['date']); ?></td>
                <td>
