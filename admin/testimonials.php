...
// Feature Update Query
  ...
  $features = get_safe_value($con,$_GET['feature']);
  $sql="UPDATE `$tblname` SET feature ='$features' WHERE ID ='$id'";
  if (!mysqli_query($con,$sql)){die('Invalid query: ' . mysqli_error($con)); } 
  echo"<META HTTP-EQUIV='REFRESH' CONTENT='0; URL=$pageurl?msg=Review%20Status%20updated%20Successfully !!'>"; exit(0);
}

//Select Query to display the records
$sql = "SELECT *,r.ID, u.ID as userid, c.ID as cityid,c.name as cityname,u.name as username,u.dp as userimage,r.status,r.date as rdate , u.email as useremail FROM `review` r
          ...
          <table id="example1_samy" class="table table-bordered table-striped table-hover" style="text-align: center;">
            ...
            <tbody>
              <?php $num=0; while($row=mysqli_fetch_array($result)){ $num=$num+1; 
                $cpic = ($row['userimage']!="") ? $row['userimage'] : "avtar.jpg";
                ?>
              <tr>
                ...
                <td align="center">
                  <img src="<?php echo UPLOAD_PATH.'users/'.$cpic; ?>" width="35" height="35" style="border-radius:50%;">
                </td>
                <td align="left">
                  <?php echo $row['username']; ?> <br>
                  <i class="fa fa-envelope text-blue"></i> <span class="text-blue bold"><?php echo $row['useremail']; ?></span>
                </td>
                <td align="left">

                  <span class="text-darkgreen bold"> City Name : </span><?php echo $row['cityname']; ?> <br> 
                ...
                <td align="left"><?php echo formatDate($row['date']); ?><br>
                </td>
                <td>
