...
if((!empty($_FILES["cover_img"])) && ($_FILES['cover_img']['error'] == 0)){
  ...
  $newfilename = "blog-".date("ymdHis");
  //Determine the path to which we want to save this file
  $ext=".".$ext;
  $newname = '../uploads/blogs/'.$newfilename.$ext;
...
if(@$_GET['mode']=="addnew"){
  $sql = "INSERT INTO `$tblname`(`status`, `tittle`, `cdate`, `date`, `description`, `pic`) VALUES 
  ('1', '$title', '$now', '$blogdate', '$desc','$pic')";
...
$img_required ="";
  $sql="UPDATE `$tblname` SET  `tittle`='$title', `description`='$desc', `date`='$blogdate' WHERE ID =$id";
  if (!mysqli_query($con,$sql)){die('Error: ' . mysqli_error($con)); }
...
//Select query when we click on pencil for edit existing entry
  ...
  if($count>0){
    while($row = mysqli_fetch_array($result)) {
      // debug($row);
      // (`ID`, `status`, `tittle`, `cdate`, `date`, `description`, `pic`)

      $get_title     = $row['tittle'];
      $get_desc      = $row['description'];
      $get_pic       = $row['pic'];
      $get_blogdate  = $row['date'];
            ...
            <?php } ?>
              <div class="box-body">
                <div class="form-group col-md-6">
                  <label for="title">Blog Title <span class="text-red">*</span></label>
                  <input type="text" class="form-control" name="title" placeholder="Blog title" value="<?= @$get_title; ?>" required autocomplete="off">
                </div>
                
                <div class="form-group col-md-6">
                  <label for="duration"> Blog Date <span class="text-red">*</span></label>
                  <input type="date" class="form-control" name="blogdate" value="<?= @$get_blogdate; ?>" autocomplete="off" required>
                </div>
                
                
                <div class="form-group col-md-12">
                  <label for="description"> Description <span class="text-red">*</span></label>
                  <textarea id="tinyeditor" name="description" rows="5" class="form-control" placeholder="Enter Blog Description"><?= @$get_desc; ?></textarea>
                </div>
                ...
                <?php if(@$_GET['mode']=="edit" && @$_GET['id']!="") {
                 ...
                 ?>
                <div class="form-group col-md-6">
                  <img src="<?= UPLOAD_PATH.'blogs/'.$get_pic; ?>" width="100" height="100" alt="" style="border: 1px solid grey;padding: 2px; border-radius: 10px;">
                </div>
              <?php endif; } ?>
              </div>
              <!-- /.box-body -->
              <div align="right" class="box-footer">
                <?php if(@$_GET['mode']=="edit" && @$_GET['id']!="") { ?>
                <button type="submit" class="<?= $btncolor; ?>"><i class="fa fa-save"></i>&nbsp; Update Blog</button>
                <?php }else{ ?>
                <button type="submit" class="<?= $btncolor; ?>"><i class="fa fa-save"></i>&nbsp; Save Blog</button>
                <?php } ?>
              </div>
            </form>
            </div>
          ...
          <table id="example1_samy" class="table table-bordered table-striped table-hover" style="text-align: center;">
            ...
            <tbody>
              <?php $num=0; while($row=mysqli_fetch_array($result)){ $num=$num+1;
                $pic = ($row['pic']!="") ? $row['pic'] : "no-img.png";
               ?>
              <tr>
                ...
                <td style="text-align: left;">
                  <span class="bold ps14"> Title: </span><?php echo $row['tittle']; ?> <br>
                  <span class="text-green bold ps14"> Blog Date:  </span><?php echo dateString($row['date']); ?> <br>
                  <span class="text-blue bold ps14"> Blog Description:  </span><?php echo $row['description']; ?> <br>
                  <!--<span class="text-blue bold ps12">URL : </span> <a href="<?= $baseurl.'/blog/'.$row['url']; ?>" target=_blank><?= $baseurl.'/blog/'.$row['url']; ?></a> <br>-->
                </td>
                <td align="center">
                  <a href="<?= UPLOAD_PATH.'blogs/'.$pic; ?>" target="_blank">
                    <img src="<?= UPLOAD_PATH.'blogs/'.$pic; ?>" width="200" height="120">
                  </a><br>
                  <?php  
                    if($row['status']=='1'){
...
<script>
  $(document).ready(function(){
    ...
    var get_scid = "<?php echo $get_scid; ?>";
