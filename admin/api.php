...
// Function to remove duplicates based on 'order_id'
function removeDuplicates($data) {
    $uniqueKeys = array();
    return array_filter($data, function ($item) use (&$uniqueKeys) {
        if (!in_array($item['order_id'], $uniqueKeys)) {
            $uniqueKeys[] = $item['order_id'];
            return true;
        }
        return false;
    });
...
else if(isset($_GET['settings']))
{
	...
	while($data=mysqli_fetch_array($result))
	{
		...
		$row1['app_min_wallet_amount'] = $data['text12'];
        $row1['status'] = $data['status'];
	   ...
	   //     	$sql_email = "SELECT * FROM user_registration where email='".$_POST['email']."' AND ID != '".$_POST['user_id']."'";
				...
				// }else{
			        ...
			        if($code_row >0){
			            ...
			            $parentuser_wallet= "INSERT INTO `wallet_history`(`wh_role`,`wh_user_id`, `wh_amount`, `description`,`wh_transaction_type`, `wh_type`, `wallet_date`, `wallet_status`) VALUES('1','".$parent_user_id."','".$child_earn."','".$parent_description."','1','1','".$cur_date."','1')";
			            ...
			            $childuser_wallet= "INSERT INTO `wallet_history`(`wh_role`,`wh_user_id`, `wh_amount`, `description`,`wh_transaction_type`, `wh_type`, `wallet_date`, `wallet_status`) VALUES('1','".$_POST['user_id']."','".$parent."','".$child_description."','1','2','".$cur_date."','1')";
...
//get all timezone
else if(isset($_GET['get_timezonev1']))
{
 			...
 			if($_POST['user_id']!='' && $_POST['payment_type']!=''  )
 			{
				...
				$date2 = date('H:i:s');
					...
					if($_POST['payment_type'] == 3)
					{
						...
						if(($wallet_num > 0 && $wallet_amt >= $_POST['final_price'] ))
						{
				  		...
				  		if($_POST['payment_type'] == 3)
				  		{
							...
							{ 
								...
								$childuser_wallet= "INSERT INTO `wallet_history`(`wh_role`,`wh_user_id`, `wh_amount`, `description`,`wh_transaction_type`, `wh_type`, `wallet_date`, `wallet_status`) VALUES('1','".$_POST['user_id']."','".$_POST['final_price']."','".$description."','2','3','".$date."','1')";
			            		$childuser_res = mysqli_query($con,$childuser_wallet);
					    ...
					    $sql_order_details = mysqli_query($con,$query_order_details)or die(mysqli_error($con));
...
else if(isset($_GET['get_order_user_upcoming']))
{
		...
		while($data = mysqli_fetch_assoc($sql))
		{
		    ...
		    $sql_rate = mysqli_query($con,$query_rate)or die(mysqli_error($con));
		    $data_rate = mysqli_fetch_array($sql_rate);
            $num_rows_rate 	= mysqli_num_rows($sql_rate);
      
	        if(empty($data_rate['rate']) )
			{
				$row2['rate'] = '0';
				$row2['comment'] = '';
			}else{
...
else if(isset($_GET['single_order']))
{
		...
		while($data = mysqli_fetch_assoc($sql))
		{
		   ...
		   $result_order = mysqli_query($con,$query_order)or die(mysqli_error($con));

		   $row_order=mysqli_fetch_assoc($result_order);
		  
		       if(empty($row_order['total_rate']))
    			{		             
    			    $row2['total_rate'] = "0";
    			    $row2['total_book'] = "0";
			...
			if(empty($data['staffid']) && empty($data['staffname']) && empty($data['staffphone']) )
            {
                $row2['staff_id']= '0';
                $row2['staff_name']= '';
                $row2['staff_phone']= '';
        ...
        //   	}
		    ...
		    $sql_order_service_details = mysqli_query($con,$query_order_service_details)or die(mysqli_error($con));
			...
			if($data['adderssid'] == '0' or empty($data['adderssid']))
			{
				$row2['a_id'] = '0';
				$row2['a_name'] = '';
				$row2['a_number'] = '';
		    ...
		    if(empty($data['stateid']) or empty($data['statename']) )
			{
			    $row2['state_id'] = "0";
				$row2['state_name'] = "0";
			}else{
			...
			if(empty($data['cityid']) or empty($data['cityname']) )
			{
			    $row2['city_id'] = "0";
				$row2['city_name'] = "";
			}else{
			...
			if(empty($data['pincodeid']) or empty($data['pincodename']) )
			{
			    $row2['pincode_id'] = "0";
				$row2['pincode_code'] = "";
			}else{
...
//order status change
else if(isset($_GET['status_manage_orderv1']))
{
	...
	$row1 = mysqli_fetch_assoc($result);
         ...
         if ($num_rows > 0)
		{
            ...
            if($row2['payment_type'] == 2)
            {
				...
				//    INSERT INTO `order_log`(`ol_id`, `ol_status`, `order_id`, `ol_msg`, `ol_slot`)
				$order_msg = "Hello! $name your booking your Booking for BonitaA services has been accepted by our service partner $restro_name";
...
//get all offer type wise
else if(isset($_GET['get_user_wallet_historyv1']))
{
  		$jsonObj4 = array();
...
// staff profile
else if(isset($_GET['staff_profilev1']))
{
...
else if(isset($_GET['reschedule_order']))
{
	...
	$num_rows = mysqli_num_rows($result);
		...
		if ($num_rows > 0)
		{
            ...
            $query_timeslot="SELECT *,timeslot.timeslot FROM `timeslot` ORDER BY timeslot.ID ASC ";
            $sql_timeslot = mysqli_query($con,$query_timeslot)or die(mysqli_error($con));
            while($data_timeslot = mysqli_fetch_assoc($sql_timeslot))
            {
	       ...
	       $result1 = mysqli_query($con,$qry11);
			...
			$order_msg = "Hello! $restro_name , $name with Booking ID $order_id has just reschedule the booking assigned you to $o_date $o_time";

			$query_log= "INSERT INTO `order_log`(`ol_status`, `order_id`, `ol_msg`, `ol_slot`)

			VALUES(1,'".$order_id."','".$order_msg."','".$date."')"; 
...
else if(isset($_GET['get_staff_cityv1']))
{
  		$jsonObj4= array();
...
//time is gone that the slot will be block and purticular date wise also 
else if(isset($_GET['get_timezone']))
{
	...
	while($data = mysqli_fetch_assoc($sql))
	{
			...
			WHERE timeslot_block.city_id='".$_POST['city_id']."' and timeslot_block.tb_date='".$_POST['date']."' AND FIND_IN_SET('".$data['timeslot']."',timeslot_block.tb_timezone) 
			...
			if($num_row_check_block){
				$row1['timezone_available']="0";	
			}else if($fetch_booking_check >= $data['slot_max_book'] ){
...
else if(isset($_GET['get_staff_city']))
{
  		$jsonObj4= array();
       	...
       	$query="SELECT *,city.ID as cityid,city.name as cityname,staff.ID as staffid,staff.name as staffname,staff.status as staffstatus,staff.dp as dp FROM staff 
	    	left join city on city.ID = staff.city_id
	    	WHERE FIND_IN_SET('".$_POST['pincode_id']."',staff.pincode_id) ORDER BY staff.ID DESC";
		...
		{	
		   ...
		   $row_order=mysqli_fetch_assoc($result_order);
            ...
            $row1['city_id'] = $data['cityid'];
            $row1['city_name'] = $data['cityname'];
			$row1['pincode_id'] = $data['pincode_id'];
            $row1['staff_phone'] = $data['mobile'];
            $row1['bb_id_card1'] = $bb_id_card1;
            $row1['bb_id_card2'] = $bb_id_card2;
            $row1['adhar_card1'] = $adhar_card1;
...
//today pending order and user select the staff
else if(isset($_GET['get_all_order_staff_pending']))
{

 		$jsonObj4= array();	

 		$query1="SELECT *,s.ID as staffid, s.name as staffname, s.mobile as staffphone,ord.ID as orderid,u.ID as userid,u.name as
		username,a.ID as adderssid,a.status as
		adderssstatus,a.type as addersstype,a.name as adderssname,a.houser_no as addersshouserno,a.adderss as
		addersses,a.pincode as addersspincode,a.latitude as addersslatitude,a.longitude as addersslongitude,a.number as
		adderssnumber,a.lendmark as addersslendmark FROM `orders` ord
		...
		while($data = mysqli_fetch_assoc($sql1))
		{
			...
			{
				$staff_id = '';
				$adress_pincode = $data['pincode'];
				$query = "and FIND_IN_SET('".$adress_pincode."',staff.pincode_id) " ;
				$qry_staff = "SELECT * FROM staff
				WHERE staff.status = 1 and staff.active_status = 1 $query
				...
				$qry_staff = "SELECT * FROM orders as o
				LEFT JOIN staff  ON staff.ID = o.staff_id
				WHERE staff.status = 1 and staff.active_status = 1 and o.staff_id = '".$staff_id."' and o.payment_type='1'  
				ORDER BY staff.ID DESC ";

			}

			
			$result_staff = mysqli_query($con,$qry_staff)or die(mysqli_error($con));
			while($data_staff = mysqli_fetch_assoc($result_staff))
			{
		...
		// Remove duplicates
		$uniqueData = removeDuplicates($jsonObj4);
...
//staff active status update
else if(isset($_GET['staff_active_status']))
{
		...
		$qry = "SELECT * FROM staff WHERE ID='".$staff_id."'"; 
		...
		$num_rows 	= mysqli_num_rows($result);
			...
			if($num_rows > 0)
			{
				...
				$delete="UPDATE `staff` SET `active_status`= '".$active_status."' WHERE `ID`='".$staff_id."'";
	  			$result1 = mysqli_query($con,$delete);

				if($result1 == 1)
					$set['JSON_DATA'][]=array('msg' => "Status updated successfully...!",'success'=>'1');
				else
					$set['JSON_DATA'][]=array('msg' => "Some error occured",'success'=>'0');
...
else if(isset($_GET['add_book']))
{
 			...
 			if($_POST['user_id']!='' && $_POST['payment_status']!=''  )
 			{
				...
				// exit;
					...
					if($_POST['payment_status'] == 3)
					{
						...
						$query_cart = "SELECT *,cart.ID as cartid,u.ID as userid,s.ID as serviceid,c.ID as catid,subcat.ID as subid,subsubcat.ID as subsubid,c.category as category_name,subcat.subcategory as sub_name,s.pic as serviceimage,s.short_description as servicedesc,s.status as servicestatus,c.status as cartstatus,s.duration as duration FROM `cart` as cart
							...
							$sql_cart = mysqli_query($con,$query_cart)or die(mysqli_error($con));
							$num_rows_cart 	= mysqli_num_rows($sql_cart);
							// echo $num_rows_cart;
							// exit;
				...
				// 		{
                            ...
                            $sql_address = mysqli_query($con,$query_address)or die(mysqli_error($con));
							...
							if(($wallet_num > 0 && $wallet_amt >= $_POST['final_price'] ))
							{
								$qry1="INSERT INTO `orders`(`repeat_user`,`user_id`, `staff_id`,`city_id`,`address`, `payment_type`, `message`, `dis_price`, `ori_price`,`conveyance_charges`,`safety_hygiene_charges`, `final_price`, `payment_status`, `order_status`, `txnid`, `mihpayid`, `payu_status`, `coupon_id`, `coupon_value`, `coupon_code`,`service_price`, `added_on`, `order_date`, `order_time`,`order_otp`)
		        				VALUES (
									'".$repeat_user."',
		        			    	'".$_POST['user_id']."',
		        			    	'".$_POST['staff_id']."',
								    ...
								    while($data1 = mysqli_fetch_array($sql1))
								   	{
							            ...
							            $sql2 = mysqli_query($con,$insertsql)or die(mysqli_error($con));
...
else if(isset($_GET['get_city']))
{
  		...
  		if($_POST['state_id'] == 0)
  		{
  			$query="SELECT *,c.ID as cityid,c.status as citystatus,c.name as cityname,s.name as statename,s.ID as stateid,s.status as statestatus,c.convenience_fee,c.safety_charge FROM `city` c
			LEFT JOIN state s ON s.ID = c.state_id
			ORDER BY c.ID DESC";
		...
		{	
            ...
            $row1['state_name'] = $data['statename'];
            $row1['city_name'] = $data['cityname'];
			$row1['convenience_fee'] = $data['convenience_fee'];
			$row1['safety_charge'] = $data['safety_charge'];
            $row1['city_status'] = $data['citystatus'];
...
// add wallet amount in staff 
else if(isset($_GET['add_wallet_transaction'])) 
{   
      if(isset($_POST['wh_amount']) && !empty($_POST['wh_amount'])){

                
                //echo 'client'; 
                $wallet_check = "SELECT * FROM staff WHERE ID = '".$_POST['wh_user_id']."' "; 
                $wallet_result = mysqli_query($con,$wallet_check);
                $wallet_row = mysqli_fetch_assoc($wallet_result);
                $wallet_amt = $wallet_row['wallet'];
                $wallet_num = mysqli_num_rows($wallet_result);
                ...
                if($wallet_num > 0)
                {
					// INSERT INTO `wallet_history`(`wh_id`, `wh_role`, `wh_user_id`, `wh_amount`, `description`, `wh_payment_id`, `wh_transaction_type`, `wh_type`, `wallet_date`, `wallet_status`)

                    date_default_timezone_set("Asia/Calcutta");   //India time (GMT+5:30)
                    $date = date("Y-m-d h:i:s");
                    
                   $description='Wallet amount ₹ '.$_POST['wh_amount'].' add successfully. ';
                   
                    $qry1_insert="INSERT INTO `wallet_history`( `wh_role`, `wh_user_id`, `wh_amount`, `description`, `wh_payment_id`, `wh_transaction_type`, `wh_type`, `wallet_date`, `wallet_status`) VALUES (
                    '2',
					'".$_POST['wh_user_id']."',
                    '".$_POST['wh_amount']."',
                    '".$description."',
                    ...
                    $result1 = mysqli_query($con,$qry1_insert);
                    ...
                    { 
                        ...
                        //type = 4 (+), type = 5 (-)
                            $final_amount = ((int)$wallet_amt + (int)$_POST['wh_amount']);
                               
                            $user_edit= "UPDATE staff SET wallet='".$final_amount."' WHERE ID = '".$_POST['wh_user_id']."'";

                            $user_res = mysqli_query($con,$user_edit);

                            if($user_res==true){

                                $set['JSON_DATA'][]=array('msg' => "Your Wallet added successfully.",'success'=>'1');
                            }else{

                                $set['JSON_DATA'][]=array('msg' => "Sorry ! Your wallet not added successfully. Please try again.",'success'=>'0');
                            }

						

                    }else{
                        $set['JSON_DATA'][]=array('msg' => "wrong staff id...!",'success'=>'0');
                    }
          ...
          $set['JSON_DATA'][]=array('msg' => "All fields are require ",'success'=>'0');
...
// staff profile
else if(isset($_GET['staff_profile1']))
{
    	
	if(isset($_POST['staff_id']) && !empty($_POST['staff_id']) )
	{
	    	
	    	$qry = "SELECT *,staff.holder_name,staff.account_number,staff.ifsc_code,city.ID as cityid,city.name as cityname,staff.ID as staffid,staff.name as staffname,staff.address as staff_address,staff.status as staffstatus,staff.wallet as staff_wallet FROM staff 
	    	left join city on city.ID = staff.city_id
	    	WHERE staff.ID='".$_POST['staff_id']."'"; 
			$result = mysqli_query($con,$qry);
			$row1 = mysqli_fetch_assoc($result);
...
else if(isset($_GET['staff_profile']))
{
	...
	if(isset($_POST['staff_id']) && !empty($_POST['staff_id']) )
	{
	    	...
	    	$qry = "SELECT *,staff.holder_name,staff.account_number,staff.ifsc_code,city.ID as cityid,city.name as cityname,staff.ID as staffid,staff.name as staffname,staff.address as staff_address,staff.status as staffstatus,staff.wallet as staff_wallet,staff.active_status as active_status FROM staff 
	    	left join city on city.ID = staff.city_id
	    	WHERE staff.ID='".$_POST['staff_id']."'"; 
			$result = mysqli_query($con,$qry);
			$row1 = mysqli_fetch_assoc($result);
		   ...
		   $row_order=mysqli_fetch_assoc($result_order);
	 	 	...
	 	 	if($fetch_email ==1)
			{
				...
				if(!empty($row1['staff_wallet'])){
					$staff_wallet = $row1['staff_wallet'];
				}else{
					$staff_wallet ="0";
				}
				...
				$set['JSON_DATA'][]	= array(
										...
										'total_book' => $total_book,
										'wallet'	=> $staff_wallet,
										'active_status'	=> $row1['active_status']
								);
			}else{
				$set['JSON_DATA'][]=array('msg' => "Client account disable...!",'success'=>'0');
	    	}
...
//staff and user wallet history
else if(isset($_GET['get_user_wallet_history']))
{
  		$jsonObj4 = array();
        if($_POST['role_type'] == 1 )
		{
			$query="SELECT *,u.name as username FROM `wallet_history` w
			LEFT JOIN user_registration u ON u.ID = w.wh_user_id
			WHERE w.wh_user_id = '".$_POST['user_id']."' and w.wh_role = '".$_POST['role_type']."'
			ORDER BY w.wh_id DESC";
		}else if($_POST['role_type'] == 2 ){
			$query="SELECT *,s.name as staffname FROM `wallet_history` w
			LEFT JOIN staff s ON s.ID = w.wh_user_id
			WHERE w.wh_user_id = '".$_POST['user_id']."' and w.wh_role = '".$_POST['role_type']."'
			ORDER BY w.wh_id DESC";
		}
		...
		{	
			...
			if($_POST['role_type'] == 1 )
			{
				...
				$row1['user_id'] = $data['wh_user_id'];
            	$row1['user_name'] = $data['staffname'];
			}
            
            $row1['wh_amount'] = $data['wh_amount'];
            $row1['description'] = $data['description'];
			$row1['wh_payment_id'] = $data['wh_payment_id'];
            $row1['wh_transaction_type'] = $data['wh_transaction_type'];
            $row1['wh_type'] = $data['wh_type'];
...
else if(isset($_GET['get_cart']))
{
		...
		$query_city="SELECT * FROM `city` c
			WHERE c.ID='".$_POST['city_id']."'
			ORDER BY c.ID DESC";

		$sql_city = mysqli_query($con,$query_city)or die(mysqli_error($con));
		
		$data_city = mysqli_fetch_assoc($sql_city);
		
		$conveyance = $data_city['convenience_fee'];
        $safety_hygiene = $data_city['safety_charge'];
		
		
		
	    $final_amount = $total1 + $conveyance + $safety_hygiene;
		...
		$set['total_dis_amount'] = "$total1";
		$set['total_ori_amount'] = "$totals_is_various1";
		$set['conveyance_charges'] = $conveyance;
		$set['safety_hygiene_charges'] = $safety_hygiene;
		$set['final_amount'] = "$final_amount";
		$set['JSON_DATA'] = $jsonObj1;	
