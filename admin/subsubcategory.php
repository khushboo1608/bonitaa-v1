<?php 
require('check.php');
$pageurl = "subsubcategory$extn";
$tblname = "subsubcategory";
...
$sub_id   = get_safe_value($con,@$_POST['subcategoryid']);
$title = get_safe_value($con,@$_POST['subsubcategory_name']);
...
if((!empty($_FILES["pic"])) && ($_FILES['pic']['error'] == 0)){
  ...
  $newfilename = "subsubcat-".date("ymdHis");
  //Determine the path to which we want to save this file
  $ext=".".$ext;
  $newname = '../uploads/subsubcategory/'.$newfilename.$ext;
...
if(@$_GET['mode']=="addnew"){
  ...
  $sql = mysqli_query($con,"SELECT * FROM `$tblname` WHERE subsubcategory_name='$title'");
  $check = mysqli_num_rows($sql);
  if($check>0){
    echo"<META HTTP-EQUIV='REFRESH' CONTENT='0; URL=$pageurl?dupl_msg=$pagename Already Exist...'>";  exit(0);
  }else{
    $sql = "INSERT INTO `$tblname`(`status`, `category`, `subcategory`, `subsubcategory_name`, `description`, `subsubcategoryurl`, `pic`, `createdon`, `rd`, `date`, `time`, `sort`) 
    VALUES ('1','$CID','$sub_id', '$title', '$desc', '$slug', '$pic', '$now', '$rd', '$dd', '$dt', '1')";
...
$img_required ="";
  $sql="UPDATE `$tblname` SET `category`='$CID', `subcategory`='$sub_id', `subsubcategory_name` = '$title' ,`description`='$desc', `subsubcategoryurl`='$slug', `modifiedon`='$now' WHERE ID =$id";
  if (!mysqli_query($con,$sql)){die('Error: ' . mysqli_error($con)); }
...
// echo $pic; 
  if($ext!=""){$pic="$newfilename$ext";
   echo $sqlx="UPDATE `$tblname` SET `pic`='$pic' WHERE ID='$id'"; 
...
//Select query when we click on pencil for edit existing entry
  ...
  if($count>0){
    while($row = mysqli_fetch_array($result)) {
      ...
      $get_subid       = $row['subcategory'];
      $get_subcategory = $row['subsubcategory_name'];
...
// $sql = "SELECT * FROM `$tblname` WHERE hide='0' ORDER BY ID DESC";
  $sql = "SELECT t1.*, t2.category,t3.subcategory as subcategory_name FROM `subsubcategory` t1 
  LEFT JOIN `category` t2 ON t2.ID=t1.category
  LEFT JOIN `subcategory` t3 ON t3.ID = t1.subcategory
  where t1.hide = 0 and t2.hide=0 and t3.hide=0
  ORDER BY t1.ID DESC";
  $result = mysqli_query($con,$sql);   
              ...
              <div class="box-body">
                ...
                <div class="form-group">
                  ...
                  <select class="form-control" name="subcategoryid" id="subcategoryid">
                    ...
                    // echo $get_cid;
                          if( @$get_cid ) {
                            $res = mysqli_query($con,"SELECT ID,subcategory FROM `subcategory` where subcategory.hide=0 and subcategory.ID='".$get_subid."'  ORDER BY ID ASC");
                            while ($row = mysqli_fetch_array($res)) {
                              if($row['ID']==$get_subid){
                ...
                <div class="form-group">
                  <label for="name">Sub Sub Category Title<span class="text-red">*</span></label>
                  <input type="text" class="form-control" name="subsubcategory_name" value="<?= @$get_subcategory; ?>" autocomplete="off" placeholder="Sub Category Title">
                </div>
                <!--<div class="form-group">-->
                ...
                <div class="form-group col-md-6">
                  ...
                  <span class="btn btn-primary margin">
                    <input type="file" name="pic" id="pic" class="img" style="width:250px;" <?= $img_required; ?>>
                  </span>
                </div>
                 <?php if(@$_GET['mode']=="edit" && @$_GET['id']!="") { ?>
                <div class="form-group col-md-3">
                  <img src="<?= UPLOAD_PATH.'subsubcategory/'.$get_pic; ?>" width="100" height="100" alt="" style="border: 1px solid grey;padding: 2px; border-radius: 10px;">
                </div>
                <?php } ?>
          ...
          <table id="example1_samy" class="table table-bordered table-striped table-hover">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td align="center">
                <?php if($row['pic'] !=""){ ?>
                  <a href="<?= UPLOAD_PATH.'subsubcategory/'.$cpic; ?>" target="_blank">
                  <img src="<?= UPLOAD_PATH.'subsubcategory/'.$cpic; ?>" width="50" height="50" style="border-radius:10%;"></a>
                <?php } ?>
                </td>
                <td>
                  <span class="text-blue bold ps12">Category:</span> <?php echo $row['category']; ?> <br>
                  <span class="text-green bold ps12">SubCategory:</span> <?php echo $row['subcategory_name']; ?> <br>
                  <span class="text-red bold ps12">Title: </span><?php echo $row['subsubcategory_name']; ?><br>
                  <span class="text-red bold ps12">Slug: </span><?php echo $row['subsubcategoryurl']; ?><br>
