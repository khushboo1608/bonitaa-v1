<?php 
require('check.php');
$pageurl      = "staffleaves$extn";
$tblname      = "staff_leaves";
$pagename     = "staffleaves";
$id           = @$_GET['id']; 
$img_required = 'required';
$sessionid    = date("ymdHis"); 

// Get POST Values
$leave_status    = get_safe_value($con,@$_POST['leave_status']);
$cancel_reason    = get_safe_value($con,@$_POST['cancel_reason']);


// Insert new entry Query
if(@$_GET['mode']=="addnew"){
    $sql = "INSERT INTO `$tblname`(`leave_status`,`type`,`category_id`,`title`, `subtitle`, `pic`, `createdon`, `sort`,`sessionid`) 
...
// Row Update Query 
  ...
  $sql="UPDATE `$tblname` SET `leave_status`='$leave_status',`cancel_reason`='$cancel_reason' WHERE ID =$id";
  if (!mysqli_query($con,$sql)){die('Error: ' . mysqli_error($con)); }
...
//Select query when we click on pencil for edit existing entry
  ...
  if($count>0){
    while($row = mysqli_fetch_array($result)) {
      // INSERT INTO `staff_leaves`(`ID`, `status`, `staff_id`, `type`, `cdate`, `message`, `leave_status`, `start_date`, `end_date`, `cancel_reason`)

      $get_staff_id    = $row['staff_id'];
      $get_type   = $row['type'];
      $get_cdate    = $row['cdate'];
      $get_message = $row['message'];
      $get_leave_status     = $row['leave_status'];
      $get_start_date  = $row['start_date'];
      $get_end_date  = $row['end_date'];
      $get_cancel_reason  = $row['cancel_reason'];
    }
  }else{
...
$sql = "SELECT *,s.name as staffname, s.ID as staffid,sl.ID,sl.type as leavestype,sl.cdate as leavescdate, sl.message as leavesmessage,sl.start_date as leavessdate,sl.end_date as leavesedate,sl.status as leavesstatus FROM staff_leaves sl
              ...
              <div class="box-body">
                ...
                <div class="form-group col-md-4">
                  ...
                  <select name="type" id="type" class="form-control" disabled>
                    ...
                    <option value="1" <?php if(@$get_type=="1"){?> selected="selected"<?php } ?>>Full Day</option>
                    <option value="2" <?php if(@$get_type=="2"){?> selected="selected"<?php } ?>>Half Day - Moring</option>
                    <option value="3" <?php if(@$get_type=="3"){?> selected="selected"<?php } ?>>Half Day - Evening</option>
                    <option value="4" <?php if(@$get_type=="4"){?> selected="selected"<?php } ?>>Multiple Day</option>
                  </select>
                </div>
                ...
                <div class="form-group col-md-4">
                  ...
                  <input type="text" class="form-control" name="title" value="<?= @$get_cdate; ?>" autocomplete="off" placeholder="Current Date" readonly>
                </div>
                <div class="form-group col-md-4">
                  <label for="name">Reason </label>
                  <input type="text" class="form-control" name="message" value="<?= @$get_message; ?>" autocomplete="off" placeholder="Reason" readonly>
                </div>
                <?php if(@$get_type == 1 or @$get_type== 2 or @$get_type == 3){ ?>
                <div class="form-group col-md-4 start_date">
                  <label for="name">Start Date <span class="text-red">*</span></label>
                  <input type="date" class="form-control" name="start_date" value="<?= @$get_start_date; ?>" autocomplete="off" placeholder="Start date" readonly>
                </div>
                <?php }else if(@$get_type == 4){ ?>
                ...
                <div class="form-group col-md-4 end_date">
                  ...
                  <input type="date" class="form-control" name="end_date" value="<?= @$get_end_date; ?>" autocomplete="off" placeholder="End date" readonly>
                </div>
                <?php } ?>
                
                <div class="form-group col-md-4">
                  <label for="type">Leaves Status </label>
                  <select name="leave_status" id="leave_status" class="form-control" >
                    <option value="">Select Type</option>
                    <!-- 1-Pending,2-approve,3-reject,4-cancel  -->
                    <option value="1" <?php if(@$get_leave_status=="1"){?> selected="selected"<?php } ?>>Pending</option>
                    <option value="2" <?php if(@$get_leave_status=="2"){?> selected="selected"<?php } ?>>Approved</option>
                    <option value="3" <?php if(@$get_leave_status=="3"){?> selected="selected"<?php } ?>>Rejected</option>
                    <option value="4" <?php if(@$get_leave_status=="4"){?> selected="selected"<?php } ?>>Cancelled</option>
                  </select>
                </div>

                  <?php if(@$get_leave_status == 4){ ?>

                    <div class="form-group col-md-4 cancel_reason" style="display:block;">
                      <label for="name">Cancel Reason <span class="text-red">*</span></label>
                      <input type="text" class="form-control" name="cancel_reason" value="<?= @$get_cancel_reason; ?>" autocomplete="off" placeholder="Cancel Reason" >
                    </div>

                  <?php }else{ ?>

                    <div class="form-group col-md-4 cancel_reason" style="display:none">
                      <label for="name">Cancel Reason <span class="text-red">*</span></label>
                      <input type="text" class="form-control" name="cancel_reason" value="<?= @$get_cancel_reason; ?>" autocomplete="off" placeholder="Cancel Reason" >
                    </div>
          ...
          <table id="example1_samy" class="table table-bordered table-striped table-hover">
            ...
            <tbody>
              ...
              while($row=mysqli_fetch_array($result)){ $num=$num+1; 
                ...
                if($row['leavesedate'] == "")
                {
                  $edate = $row['leavessdate'];
                }else{
                  $edate = $row['leavesedate'];
                }
                $sdate = $row['leavessdate'];



                $query_count = "SELECT t.*, datediff(greatest(t.end_date, '$edate'), least(t.start_date, '$sdate')) + 1 as total_days from staff_leaves t where t.ID = '".$row['ID']."' ";
              ...
              <tr>
                ...
                <td valign="middle">
                  <span class="text-green bold font-15">Type: </span><span><?php  if($row['leavestype'] == 1)
                    { 
                      $type = 'Full Day';
                     echo $total_days = $type.' - '.$total_count.' days';

                    }else if($row['leavestype'] == 2){
                      $type ='Half Day - Moring';
                     echo $total_days = $type;
                    }else if($row['leavestype'] == 3){
                      $type ='Half Day - Evening';
                     echo $total_days = $type;
                    }else if($row['leavestype'] == 4){
                      $type ='Full Day';
                      echo $total_days = $type.' - '.$total_count.' days';
                    }         
                  ?></span><br>
                ...
                <td><?php if($row['leave_status'] == 1) {echo 'Pending'; }else if($row['leave_status'] == 2) {echo 'Approved'; }else if($row['leave_status'] == 3) {echo 'Rejected'; }else if($row['leave_status'] == 4) {echo 'Cancelled'; } ?></td>
