...
$SID     = get_safe_value($con,@$_POST['servicesid']);
...
// Image Upload Code
$ext="";
if((!empty($_FILES["url"])) && ($_FILES['url']['error'] == 0)){
  $filename    = strtolower(basename($_FILES['url']['name']));
  ...
  move_uploaded_file($_FILES['url']['tmp_name'],$newname);  
} 
if($ext==""){$pic="";} else {$pic="$newfilename$ext";} 

// Insert new entry Query
if(@$_GET['mode']=="addnew"){
  $sql = "INSERT INTO `$tblname`(`status`, `category`, `subcategory`, `services`, `title`, `url`, `amount`, `description`, `date`, `hide`) VALUES ('1', '$CID', '$SCID', '$SID' ,'$title', '$url', '$amt', '$desc', '$now', '1')";
...
$img_required ="";
  $sql="UPDATE `$tblname` SET `category`='$CID', `subcategory`='$SCID', services='$SID', `title`='$title', `url`='$surl', `amount`='$amt', `description`='$desc' WHERE ID =$id";
  if (!mysqli_query($con,$sql)){die('Error: ' . mysqli_error($con)); }
...
if((!empty($_FILES["url"])) && ($_FILES['url']['error'] == 0)){
  ...
  if($ext!=""){$pic="$newfilename$ext";
    $sqlx="UPDATE `$tblname` SET `url`='$pic' WHERE ID='$id'"; 
...
// $sql = "SELECT t1.*, t2.category FROM `$tblname` t1 INNER JOIN `category` t2 ON t2.ID=t1.category WHERE t1.hide='0' ORDER BY t1.ID DESC";
 ...
 $sql = "SELECT t1.*, t2.category, t3.subcategory,t4.title FROM `$tblname` t1 
      INNER JOIN `category` t2 ON t2.ID=t1.category 
      LEFT JOIN `subcategory` t3 ON t3.ID = t1.subcategory 
      LEFT JOIN `services` t4 ON t4.ID = t1.services 
      Where t1.hide ='0'
      ORDER BY t1.ID DESC";
$result = mysqli_query($con,$sql);   
              ...
              <div class="box-body">
                ...
                <div class="form-group col-md-4">
                  ...
                  <select class="form-control" name="subcategoryid" id="subcategoryid">
                    ...
                    // echo $get_cid;
                          if( @$get_cid ) {
                            $res = mysqli_query($con,"SELECT ID,subcategory FROM `subcategory` ORDER BY ID ASC");
                            while ($row = mysqli_fetch_array($res)) {
                              if($row['ID']==$get_scid){
                ...
                <div class="form-group col-md-4">
                  <label for="servicesid">Services <span class="text-red">*</span></label>
                  <select class="form-control" name="servicesid" id="servicesid">
                    <option>Select Services</option>
                    <?php
                        if( @$get_scid ) {
                            $res = mysqli_query($con,"SELECT ID,title FROM `services` ORDER BY ID ASC");
                            while ($row = mysqli_fetch_array($res)) {
                              if($row['ID']==$get_sid){
                ...
                <div class="form-group col-md-3">
                  <label for="mrp"> Price <span class="text-red">*</span></label>
                  <input type="number" class="form-control" name="amount" value="<?= @$get_amount; ?>" placeholder="Enter Price"  autocomplete="off" required>
                </div>
                     
                <div class="form-group col-md-12">
                  <label for="description"> Description <span class="text-red">*</span></label>
                  <textarea id="tinyeditor" name="description" rows="5" class="form-control" placeholder="Enter Course Description"><?= @$get_desc; ?></textarea>
                </div> 
                <div class="form-group col-md-6">
                  ...
                  <span class="btn btn-primary margin">
                    <input type="file" name="url" id="cover_img" class="img" style="width:250px;" <?= $pic_required; ?>>
                  </span>
                </div>
                <?php if(@$_GET['mode']=="edit" && @$_GET['id']!="") { ?>
                <div class="form-group col-md-6">
          ...
          <table id="example1_samy" class="table table-bordered table-striped table-hover" style="text-align: center;">
            ...
            <tbody>
              <?php $num=0; while($row=mysqli_fetch_array($result)){ $num=$num+1;
                $pic = ($row['url']!="") ? $row['url'] : "noimg.webp";
               ?>
              <tr>
                ...
                <td style="text-align: left;">
                  <span class="text-red bold ps13">Price: </span> <span class="ps14 bold"><?php echo '&#8377; '.number_format($row['amount']).'/-'; ?></span> <br>
                
                </td>
                <td title="Modified on:"><?php echo $row['date']; ?></td>
                <td>
...
<script>
  $(document).ready(function(){
    ...
    var get_sid = "";
    console.log("Services ID: "+ get_sid);

      var id=jQuery(this).val();
      console.log("SubCategory ID: "+ id);
      if(id=='-1'){
        jQuery('#servicesid').html('<option value="-1">Select Services </option>');
      }else{
        $("#divLoading").addClass('show');
        jQuery('#servicesid').html('<option value="-1">Select Service </option>');
        jQuery.ajax({
          type:'post',
          url:'get_data1.php?mode=services',
          data:'id='+id+'&get_sid='+get_sid,
          // data:'id='+id,
          success:function(result){
            $("#divLoading").removeClass('show');
            jQuery('#servicesid').append(result);
          }
        });
      }
    });
