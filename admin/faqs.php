...
if(@$_GET['mode']=="addnew"){
    $sql = "INSERT INTO `$tblname`(`status`, `question`, `answer`, `createdon`, `sort`) 
    VALUES ('1', '$question', '$answer', '$now', '$sort')";
...
$img_required ="";
  $sql="UPDATE `$tblname` SET `question`='$question', `answer`='$answer', `modifiedon`='$now' WHERE ID =$id";
  if (!mysqli_query($con,$sql)){die('Error: ' . mysqli_error($con)); }
...
//Select query when we click on pencil for edit existing entry
  ...
  if($count>0){
    while($row = mysqli_fetch_array($result)) {
      $get_question = $row['question'];
      $get_answer   = $row['answer'];
      $get_sort     = $row['sort'];
    }
  }else{
            ...
            <?php } ?>
              <div class="box-body">
                <div class="form-group">
                  <label for="name">Question <span class="text-red">*</span></label>
                  <input type="text" class="form-control" name="question" value="<?= @$get_question; ?>" autocomplete="off" placeholder="FAQ Question">
                </div>
                <div class="form-group">
                  <label for="desc">Answer <span class="text-red">*</span></label>
                  <textarea name="answer" id="details" rows="5" class="form-control" placeholder="Enter FAQ Answer Here"><?=  @$get_answer; ?></textarea>
            ...
            <tbody>
              ...
              <tr>
                ...
                <td class="text-justify">
                  <span class="bold font-14"><?php echo $row['question']; ?></span> &nbsp;
                  <span data-toggle='tooltip' title='Sort value' style='background-color: #CCCCCC; padding:3px 5px; color: #666666; border-radius:50%;'><?= $row['sort'] ?></span> <br>
                  <span class="font-13"><?php echo $row['answer']; ?></span>
                </td>
