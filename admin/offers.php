...
if((!empty($_FILES["pic"])) && ($_FILES['pic']['error'] == 0)){
  ...
  $newfilename = "offers-".date("ymdHis");
  //Determine the path to which we want to save this file
  $ext=".".$ext;
  $newname = '../uploads/offers/'.$newfilename.$ext;
...
if(@$_GET['mode']=="addnew"){
 $sql = "INSERT INTO `$tblname`(`status`,`type`,`category_id`,`subcategory_id`, `subsubcategory_id`, `title`,`click`,`pic`, `createdon`,`sort`) 
    VALUES ('1', '$type', '$category_id', '$sub_category_id', '$sub_sub_sub_category_id','$title','$click', '$pic', '$now', '$sort')";
...
$img_required ="";
    $sql="UPDATE `$tblname` SET `title`='$title',`type`='$type' ,`click`='$click',`category_id`='$category_id',`subcategory_id`='$sub_category_id',`subsubcategory_id`='$sub_sub_sub_category_id', `sort`='$sort', `modifiedon`='$now' WHERE ID =$id";
  if (!mysqli_query($con,$sql)){die('Error: ' . mysqli_error($con)); }
...
$sql = "SELECT *,c.ID,subcat.ID,subsubcat.ID,c.category as cname,o.pic as oimage,o.ID,o.status,subcat.subcategory as subcat_name,subsubcat.subsubcategory_name as subsubsubcat_name,o.modifiedon FROM `offers` o 
...
WHERE o.hide = 0 ORDER BY o.ID DESC";
$result = mysqli_query($con,$sql);   
              ...
              <div class="box-body">
                ...
                <div class="form-group col-md-4">
                  ...
                  <select name="type" id="type" class="form-control">
                    ...
                    <option value="1" <?php if(@$get_type=="1"){?> selected="selected"<?php } ?>>Offers</option>
                    <option value="2" <?php if(@$get_type=="2"){?> selected="selected"<?php } ?>>Hot Deals</option>
                    <!--<option value="3" <?php if(@$get_type=="3"){?> selected="selected"<?php } ?>>Super Saving </option>-->
                  </select>
                </div>
                ...
                <div class="form-group col-md-3">
                  <img src="<?= UPLOAD_PATH.'offers/'.$get_pic; ?>" width="200" height="100" alt="" style="border: 1px solid grey;padding: 2px; border-radius: 10px;">
                </div>
                <?php } ?>
          ...
          <table id="example1_samy" class="table table-bordered table-striped table-hover" style="text-align: center;">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td align="left">
                  <span class="text-green bold font-15">Type: </span><span><?php if($row['type'] == 0){echo 'None';}else if($row['type'] == 1){echo 'Offers';}else if($row['type'] == 2){echo 'Hot Deals';} ?></span><br>
                ...
                <td align="center">
                  <a href="<?= UPLOAD_PATH.'offers/'.$pic; ?>" target="_blank">
                    <img src="<?= UPLOAD_PATH.'offers/'.$pic; ?>" width="200" style="max-height: 150px;">
                  </a>
                </td>
